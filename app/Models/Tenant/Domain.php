...
class Domain extends BaseDomain
{
    ...
    public function scopeAllowedEmailsByTenant(Builder $query, string $email): ?Model
    {
        return $this->select(['tenant_id', 'allowed_email_domain'])
            ->with('tenant')
            ->where(
                'allowed_email_domain',
                substr(strrchr($email, '@'), 1)
            )
            ->whereNotNull('allowed_email_domain')
            ->first();
    }
}
